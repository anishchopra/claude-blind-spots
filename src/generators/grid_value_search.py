"""Generator for grid/table value search tasks."""
...
class GridValueSearchOutput(BaseModel):
    cell: str  # The cell reference (e.g., "B2")


class GridValueSearchGenerator(BaseGenerator):
    """Generate grid images to test finding which cell contains a given value."""

    task_name = "grid_value_search"
    output_model = GridValueSearchOutput
    ...
    def generate_one(self, sample_id: str, **params) -> tuple[Path, Path]:
        """Generate a grid value search test image."""
        ...
        # Generate unique random values for each cell
        total_cells = rows * cols
        # Use a range large enough to ensure uniqueness
        # For larger grids, use 3-digit numbers
        if total_cells <= 90:
            all_values = random.sample(range(10, 100), total_cells)
        else:
            all_values = random.sample(range(100, 1000), total_cells)
        values = []
        idx = 0
        for _ in range(rows):
            row_values = []
            for _ in range(cols):
                row_values.append(all_values[idx])
                idx += 1
        ...
        # Save
        prompt = f"Which cell contains the value {target_value}?"
        ground_truth = GridValueSearchOutput(cell=cell_name)
        generation_params = {
            "rows": rows,
            "cols": cols,
